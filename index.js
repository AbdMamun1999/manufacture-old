...
async function run() {
  try {
    ...
    const paymentCollection = client
      .db("manufacturer-project")
      .collection("payment");
    const productForCollection = client
      .db("manufacturer-project")
      .collection("product");
    ...
    const verifyAdmin = async (req, res, next) => {
      ...
      const requseterAccount = await userCollection.findOne({
        email: requester,
      });
    ...
    app.patch("/orders/:id", async (req, res) => {
      ...
      const updatedOrders = await purchaseCollection.updateOne(
        filter,
        updateDoc
      );
